# Example Tool Tutorial

This document shows how to create a simple tool module step-by-step.

## Example 1: Simple Text Statistics Tool

This tool module adds functionality to analyze text files and provide statistics.

```python
"""
Text statistics tool for analyzing file content.
...
This tool helps understand project organization.

```python
"""
Project structure analyzer tool.
...
This example shows how to call external commands.

```python
"""
Code quality checker tool using external tools.
...
1. **Create the tool file**: Save your tool as `tools/my_tool.py`

2. **Start llode**: It will automatically discover and load your tool
   ```
   Loading plugins...
     ✓ my_tool
   Loaded 1 plugin(s)
...
3. **Check tool status**: Use `/plugins` command
   ```
   You: /plugins
   
   Plugin Status:
   Loaded plugins:
     • my_tool: Description from tool docstring
   ```
...
5. **Dependencies**: Document required packages in tool docstring
...
db_path = git_root / ".llode" / "my_tool.db"
